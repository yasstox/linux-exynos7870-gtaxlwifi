...
static int stmfts_legacy_read_one_event(struct stmfts_data *sdata,
...
static int stmfts_legacy_system_reset(struct stmfts_data *sdata)
{
	static const u8 reset_cmd[] = { 0xb6, 0x00, 0x23, 0x01 };
	u8 event[STMFTS_EVENT_SIZE];
	int ret;
	int i;

	ret = i2c_master_send(sdata->client, reset_cmd, sizeof(reset_cmd));
	if (ret < 0)
		return ret;
	...
	for (i = 0; i < 20; i++) {
		ret = stmfts_legacy_read_one_event(sdata, event);
		if (ret)
			return ret;

		if (event[0] == STMFTS_EV_CONTROLLER_READY ||
		    event[0] == STMFTS_EV_SLEEP_OUT_CONTROLLER_READY)
			return 0;
...
static int stmfts_legacy_enable_interrupts(struct stmfts_data *sdata)
{
	static const u8 irq_cmd[] = { 0xb6, 0x00, 0x1c, 0x41 };
	int ret;

	ret = i2c_master_send(sdata->client, irq_cmd, sizeof(irq_cmd));
	if (ret < 0)
		return ret;

	return ret == sizeof(irq_cmd) ? 0 : -EIO;
}

static int stmfts_legacy_start(struct stmfts_data *sdata)
{
	int err;

	err = i2c_smbus_write_byte(sdata->client, STMFTS_SLEEP_OUT);
	if (err)
		return err;
	...
	if (sdata->use_key) {
		...
		if (err)
			dev_warn(&sdata->client->dev,
				 "failed to enable touchkey: %d\n", err);
	}

	return stmfts_legacy_enable_interrupts(sdata);
}
...
static int stmfts_input_open(struct input_dev *dev)
{
	...
	if (sdata->legacy_protocol) {
		err = stmfts_legacy_start(sdata);
		if (err) {
...
static int stmfts_power_on(struct stmfts_data *sdata)
{
	 ...
	 * that the reset time is < 10ms, sleep 20ms to be safe.
	 */
	msleep(20);
	...
	if (sdata->legacy_protocol) {
		/*
		 * Older FTS controllers used by gtaxlwifi use the Samsung
		 * legacy register protocol. The generic 0xa0 reset command
		 * is accepted on I2C but never signals command completion.
		 */
		err = stmfts_legacy_system_reset(sdata);
		if (err)
			return err;
		...
		/*
		 * Drain status/error events generated by wake-up before the
		 * Linux IRQ handler starts consuming the event FIFO.
		 */
		for (i = 0; i < 8; i++) {
			err = stmfts_legacy_read_one_event(sdata, event);
			if (err)
				return err;

			if (event[0] == STMFTS_EV_NO_EVENT)
				break;
		}
		...
		/*
		 * Keep the controller asleep until an input consumer opens
		 * the device. stmfts_legacy_start() will wake it and enable
		 * touch sensing plus the controller's internal interrupts.
		 */
...
static int stmfts_probe(struct i2c_client *client)
{
	...
	sdata->legacy_protocol = device_property_read_bool(&client->dev,
							   "st,legacy-protocol");
...
static int stmfts_resume(struct device *dev)
{
	...
	ret = stmfts_power_on(sdata);
	if (ret || !sdata->legacy_protocol)
		return ret;
	...
	if (running)
		return stmfts_legacy_start(sdata);
